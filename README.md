...
- 本リポジトリ内のプログラムにおける自己位置推定処理は、プレティア・テクノロジーズ社がクラウド上で一時的に管理、提供するPretiaサーバーにて処理を実施します。
- Pretiaサーバー及びその機能は予告なく変更または削除する可能性がございます。そのため、本リポジトリ公開中であってもPretiaサーバーの変更または削除によりプログラムが動作不可となる場合がございます。
